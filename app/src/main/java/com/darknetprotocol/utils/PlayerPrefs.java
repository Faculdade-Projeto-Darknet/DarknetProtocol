...
public class PlayerPrefs {
    ...
    public void addXp(int amount) {
        int currentXp = getXp();
        preferences.edit().putInt(KEY_XP, currentXp + amount).apply();
    }

    public void setXp(int xp) {
        preferences.edit().putInt(KEY_XP, xp).apply();
    }

    public boolean isMission1Completed() {
        return preferences.getBoolean(KEY_MISSION_1, false);
    }

    public void setMission1Completed(boolean completed) {
        preferences.edit().putBoolean(KEY_MISSION_1, completed).apply();
    }

    public boolean isMission2Completed() {
        return preferences.getBoolean(KEY_MISSION_2, false);
    }

    public void setMission2Completed(boolean completed) {
        preferences.edit().putBoolean(KEY_MISSION_2, completed).apply();
    }
    ...
    public String getNickname() {
        return preferences.getString(KEY_NICKNAME, "Ghost_404");
    }
    ...
    public void setProfileImage(String imageUri) {
        preferences.edit().putString(KEY_PROFILE_IMAGE, imageUri).apply();
    }
